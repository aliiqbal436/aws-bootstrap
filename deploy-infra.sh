...
aws cloudformation deploy \
  ...
  --no-fail-on-empty-changeset \
  --capabilities CAPABILITY_NAMED_IAM \
  --parameter-overrides EC2InstanceType=$EC2_INSTANCE_TYPE GitHubOwner=$GH_OWNER GitHubRepo=$GH_REPO GitHubBranch=$GH_BRANCH GitHubPersonalAccessToken=$GH_ACCESS_TOKEN CodePipelineBucket=$CODEPIPELINE_BUCKET
